...
@section('content')
    <div class="flex flex-col m-10 text-white space-y-10">
        ...
        <form action="{{ route('watchlist-search') }}" method="POST" class="w-full">
            @csrf
            <input type="text" name="name" class="rounded-md border-none bg-gray-900 w-full"
                    ...
                    @foreach ($watchlists as $watchlist)
                        <tr class="bg-gray-900">
                            <td scope="row" class="px-6 py-4 w-60">
                                <img src="{{ asset('storage/' . $watchlist->movie->thumbnail) }}"
                                    alt="{{ $watchlist->movie->name }}" class="rounded-md">
                            </td>
                            ...
                            <td class="px-6 py-4 text-lg font-medium">
                                <button data-modal-target="default"
                                    data-modal-toggle="defaultModal-{{ $watchlist->id }}">...</button>
                                <!-- Main modal -->
                                <div id="defaultModal-{{ $watchlist->id }}" tabindex="-1" aria-hidden="true"
                                        ...
                                        <div class="relative bg-gray-900 text-white rounded-lg shadow">
                                            ...
                                            <div class="flex items-start justify-between p-4 rounded-t">
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-modal-hide="defaultModal-{{ $watchlist->id }}">
                                            ...
                                            <form action="/watchlist" method="POST"
                                                class="flex flex-col justify-between mt-8">
                                                @csrf
                                                <input type="hidden" name="watchlist_id" value="{{ $watchlist->id }}">
                                                <select name="status" class="rounded-md border-none bg-gray-600">
                                                ...
                                                <div class="flex flex-row justify-end mt-10 space-x-8">
                                                    ...
                                                    <button
                                                        class="py-4 px-2 bg-red-600 rounded-md hover:bg-red-800 transition-colors duration-300 ease-in-out">Save
                                                        changes</button>
                                                </div>
                                            </form>
                                        </div>
